...
struct PauseWarningSheetView: View {
    @Binding var isPresented: Bool
    let onConfirmPause: () -> Void
    ...
    var body: some View {
        VStack(spacing: 32) {
            VStack(spacing: 8) {
                ...
                VStack(spacing: 20) {
                    Text("Завершите все активные заказы,\nпосле этого начнётся пауза")
                        .headline2Style()
                        .foregroundStyle(Color.text1)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)

                    Text("Берите паузу не дольше 5 минут,\nв это время новые заказы не будут\nназначаться")
                        .headline2Style()
                        .foregroundStyle(Color.text1)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
            ...
            .buttonStyle(PauseWarningButtonStyle())
...
private struct PauseWarningButtonStyle: ButtonStyle {
...
#Preview {
    Color.surface0
        ...
        .sheet(isPresented: .constant(true)) {
            PauseWarningSheetView(isPresented: .constant(true), onConfirmPause: {})
        }
}
